...
import org.team5924.frc2025.subsystems.climber.ClimberIOInputsAutoLogged;
  ...
  public void setGoalState(ClimberState newGoal) {
    ...
    switch (goalState) {
      case CLIMB:
        ...
        // READY_TO_CLIMB
        if (RobotState.getInstance().getClimberStateOLD().equals(ClimberState.STOW)) {
          ...
          RobotState.getInstance().setClimberStateOLD(ClimberState.CLIMB);
          break;
        }

      case STOW:
        RobotState.getInstance().setClimberStateOLD(ClimberState.STOW);
        break;

      case READY_TO_CLIMB:
        RobotState.getInstance().setClimberStateOLD(ClimberState.READY_TO_CLIMB);
        break;
      ...
      case REVERSE_CLIMB:
        if (RobotState.getInstance().getClimberStateOLD().equals(ClimberState.STOW)) {
          ...
          RobotState.getInstance().setClimberStateOLD(ClimberState.REVERSE_CLIMB);
          break;
        }
    }

    // if the goal state and the actual state are not equal, then there was an error
    invalidStateTransition.set(RobotState.getInstance().getClimberStateOLD() != goalState);
  }
